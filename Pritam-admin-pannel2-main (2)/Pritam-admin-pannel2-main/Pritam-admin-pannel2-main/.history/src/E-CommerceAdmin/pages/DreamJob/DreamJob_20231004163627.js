...
const DreamJob = () => {
  ...
  const deleteHandler = async () => {
    try {
        const { data } = await axios.delete(`${Baseurl}api/v1/admin/dream/deleteYourDreamsQuickly/64b266fdab33afa1ef612a95`)
    } catch (e) {
      const msg = e.response.data.message;
      Store.addNotification({
        title: "",
      ...
      <section className="sectionCont">
        <div className="pb-4  w-full flex justify-between items-center">
          ...
          <div>
            ...
            <button className="md:py-2 px-3 md:px-4 py-1 rounded-sm bg-[#d11a2a] text-white tracking-wider ml-2">
              Delete
            </button>
          </div>
        </div>
