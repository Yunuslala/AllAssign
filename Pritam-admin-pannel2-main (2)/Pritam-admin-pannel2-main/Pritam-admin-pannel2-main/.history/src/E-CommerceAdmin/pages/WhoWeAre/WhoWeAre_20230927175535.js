...
const WhoWeAre = () => {
  ...
  const fetchData = async () => {
    try {
      const { data } = await axios.get(`${Baseurl}api/v1/admin/getWhoWeare`);
      setData(data.data);
  ...
  const deleteHandler = async (id) => {
    try {
      const { data } = await axios.delete(
        `${Baseurl}api/v1/admin/DeleteWhoWeare/${id}`,
        Auth
      );
  ...
  function MyVerticallyCenteredModal(props) {
    ...
    const [secondTitle, setSecondTitle] = useState(editData?.desc?.[0]?.title);
    const [desc, setDesc] = useState(editData?.desc?.[0]?.title);
    ...
    const postHandler = async (e) => {
      ...
      try {
        const data = await axios.post(
          `${Baseurl}api/v1/admin/addWhoWeare`,
          payload,
          Auth
        );
        Store.addNotification({
    ...
    const putHanlder = async (e) => {
      ...
      try {
        const data = await axios.put(
          `${Baseurl}api/v1/admin/updateWhoWeare/${id}`,
          payload,
          Auth
        );
        Store.addNotification({
    ...
    return (
      ...
      >
        ...
        <Modal.Body>
          <Form onSubmit={edit ? putHanlder : postHandler}>
            {imageLoading === true ? (
